...
function App() {
  ...
  const [users, setUsers] = useState(() => {
    try { const s = localStorage.getItem("okutan_users"); return s ? JSON.parse(s) : initialUsers; }
    catch { return initialUsers; }
  });
  const [currentUser, setCurrentUser] = useState(() => {
    try { const s = localStorage.getItem("okutan_current_user"); return s ? JSON.parse(s) : null; }
    catch { return null; }
  });
  ...
  const [leads, setLeads] = useState(() => {
    try { const s = localStorage.getItem("okutan_leads"); return s ? JSON.parse(s) : initialLeads; }
    catch { return initialLeads; }
  });
  const [students, setStudents] = useState(() => {
    try { const s = localStorage.getItem("okutan_students"); return s ? JSON.parse(s) : initialStudents; }
    catch { return initialStudents; }
  });
  const [teachers, setTeachers] = useState(() => {
    try { const s = localStorage.getItem("okutan_teachers"); return s ? JSON.parse(s) : initialTeachers; }
  ...
  // ── Persist ─────────────────────────────────────────────────────────────
  useEffect(() => { localStorage.setItem("okutan_users", JSON.stringify(users)); }, [users]);
  useEffect(() => {
    currentUser
      ? localStorage.setItem("okutan_current_user", JSON.stringify(currentUser))
      : localStorage.removeItem("okutan_current_user");
  }, [currentUser]);
  ...
  const handleLogin = (e) => {
    ...
    if (found) {
      ...
      showToast(`Hoş geldiniz, ${found.name}!`, "success");
    } else {
  ...
  const handleRegister = (e) => {
    ...
    if (adminCode !== ADMIN_REGISTER_PASSWORD) {
      showToast("Yönetici şifresi hatalı! Yeni hesap açamazsınız.", "error"); return;
    }
    if (users.some(u => u.username.toLowerCase() === username.trim().toLowerCase())) {
      showToast("Bu kullanıcı adı zaten alınmış!", "error"); return;
    }
    const newUser = { id: `u-${Date.now()}`, name: name.trim(), username: username.trim(), password, role };
    setUsers(prev => [...prev, newUser]);
    setCurrentUser(newUser);
    setRegisterForm({ name: "", username: "", password: "", role: "Asistan", adminCode: "" });
    setActiveTab("dashboard");
    showToast(`Hesap oluşturuldu! Hoş geldiniz, ${newUser.name}.`, "success");
  };
  ...
  const autofillLogin = (username, password) => {
    const found = users.find(u => u.username === username && u.password === password);
    if (found) {
      setCurrentUser(found);
      setActiveTab("dashboard");
      showToast(`Test Girişi: ${found.name} (${found.role})`, "success");
    }
  };
  ...
  const handleEditLead = (e) => {
    ...
    setLeads(prev => prev.map(l =>
      l.id === selectedLead.id
        ? { ...selectedLead, updatedDate: new Date().toISOString().split("T")[0] }
        : l
    ));
    setModal(null);
  ...
  const handleConfirmRegistration = (e) => {
    ...
    const price = parseFloat(confirmForm.totalPrice) || 0;
    const disc = parseFloat(confirmForm.discount) || 0;
    const firstPay = parseFloat(confirmForm.firstPayment) || 0;
    ...
    const newStudent = {
      ...
      discount: disc,
      paidAmount: firstPay,
      paymentStatus: firstPay >= finalPrice ? "paid" : firstPay > 0 ? "partial" : "unpaid",
      ...
      notes: [{
        ...
        content: `Kesin kayıt yapıldı. Veli: ${selectedLead.name} | Öğrenci: ${confirmForm.studentName.trim()} (${confirmForm.studentAgeGrade}). Öğretmen: ${selectedTeacher.name}.`
      }],
      payments: firstPay > 0 ? [{
        id: `p-${Date.now()}`,
        date: new Date().toISOString().split("T")[0],
        amount: firstPay,
        type: confirmForm.firstPaymentType,
        description: "Kayıtta alınan peşinat"
      }] : []
    };
  ...
  const handleDeleteNote = (studentId, noteId) => {
    if (currentUser?.role === "Öğretmen") {
  ...
  const handleAddPayment = (e) => {
    ...
    const updated = students.map(s => {
      if (s.id !== paymentForm.studentId) return s;
      const newPaid = s.paidAmount + amount;
      const finalPrice = s.totalPrice - s.discount;
      const newStatus = newPaid >= finalPrice ? "paid" : newPaid > 0 ? "partial" : "unpaid";
      const newPayment = { id: `p-${Date.now()}`, date: paymentForm.date, amount, type: paymentForm.type, description: paymentForm.description };
      const newNote = {
        id: `n-${Date.now()}`,
        date: paymentForm.date,
        author: `${currentUser.name} (${currentUser.role})`,
        content: `${amount.toLocaleString()} TL tahsil edildi. (${paymentForm.type} — ${paymentForm.description})`
      };
      return { ...s, paidAmount: newPaid, paymentStatus: newStatus, payments: [newPayment, ...s.payments], notes: [newNote, ...s.notes] };
    });

    setStudents(updated);
    if (selectedStudent?.id === paymentForm.studentId) {
      setSelectedStudent(updated.find(s => s.id === paymentForm.studentId));
    }
    setModal(null);
